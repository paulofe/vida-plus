...
async function pullCampaign(){
  ...
  const from=state.config.startCampaign||toKey();
  const { data, error } = await supabaseClient.from('days').select('date,data,updated_at').eq('user_id', user.id).gte('date', from).order('date',{ascending:true}); if(error){console.error('Supabase SELECT error', error);}
